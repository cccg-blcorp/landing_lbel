...
import { loaderPageOff, loaderPageOn, buttonLoadingOn, buttonLoadingOff } from '../helpers/loader'
...
const saveStep = async (inputOptions) => {
	...
	let options = {
		...
		idSection: null,
		navigation: null,
		btnText: 'Guardar y Continuar <span class="icon icon-arrow-right text-size-xl ml-1"></span>'
	}
...
export const renderSteps = () => {
	...
	const stepButtons = Array.from(document.querySelectorAll('[data-toggle="button-step"]'))
	...
	stepButtons.forEach(button => {
		...
		button.addEventListener('click', async (event) => {
			...
			if (idButton === 'btn-register') {
				...
				isValidate = validateFields(idSection, 'nm:required,ln:required,e:required|email,pn:required|number|phone,ot:checked')
			...
			if (idButton === 'btn-step1a') {
				...
				isValidate = validateFields(idSection, 'pn:required|number|phone,ot:checked')
				if (__isDev) console.log(`validation-#${idSection}:`, isValidate)
			...
			if (idButton === 'btn-step3') {
				...
				updateState(idSection, {
					p1: (getState().p1 === '') ? document.querySelector('input[name="p1"]').value : getState().p1
				})
			...
			if (idButton === 'btn-step4') {
				...
				updateState(idSection, {
					p2: (getState().p2 === '') ? document.querySelector('input[name="p2"]').value : getState().p2
				})
