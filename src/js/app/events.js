...
export const startDataLayerSession = (uid, location) => {
	...
	if (isAllowedSession === falseSession) {
		...
		if (__isDev)
			console.log(`events-data-layer-session: start on ${location}`, data);
		...
		if (!localStorage.getItem("idCDC") || localStorage.getItem("idCDC") === "")
			localStorage.removeItem("idCDC");
		localStorage.setItem("idCDC", idCDC);
	} else {
